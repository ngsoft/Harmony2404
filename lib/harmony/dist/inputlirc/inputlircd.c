...
static void processevent(evdev_t *evdev, fd_set *permset) {
	...
	if(KEY_NAME[event.code])
		len = snprintf(message, sizeof message, "%x %x %s%s%s%s%s %s\n", event.code, repeat, ctrl ? "CTRL_" : "", shift ? "SHIFT_" : "", alt ? "ALT_" : "", meta ? "META_" : "", KEY_NAME[event.code], remote);
	else
		len = snprintf(message, sizeof message, "%x %x KEY_CODE_%d %s\n", event.code, repeat, event.code, remote);
...
int main(int argc, char *argv[]) {
	...
	while((opt = getopt(argc, argv, "cd:gm:x:n:fu:r:t:")) != -1) {
                switch(opt) {
					case 'd':
